...
def valid_filtered_outages(valid_filtered_outages_dt, valid_site_info):
    return filter_outages_by_site_info(
        filtered_outages_dt=valid_filtered_outages_dt, site_info_data=valid_site_info
    )
...
def test_get_outages_invalid_api_url_and_invalid_key(
    invalid_api_url, invalid_headers, requests_session
):
    # Try to issue a get request using the wrong API endpoint & API Key
    response = get_outages(
        api_endpoint_url=invalid_api_url,
...
def test_get_outages_valid_api_url_and_invalid_key(
    valid_api_url, invalid_headers, requests_session
):
    # Try to issue a get request using the correct API endpoint & incorrect API Key
    response = get_outages(
        api_endpoint_url=valid_api_url,
        headers=invalid_headers,
        requests_session=requests_session,
    )
...
def test_get_outages_invalid_api_url_and_valid_key(
    invalid_api_url, valid_headers, requests_session
):
...
def test_get_outages_valid_api_url_and_valid_key(
    valid_api_url, valid_headers, requests_session
):
...
def test_get_site_info_invalid_api_url_and_invalid_key(
    invalid_api_url, invalid_headers, requests_session
):
    # Try to issue a get request using the wrong API endpoint & API Key
    response = get_site_info(
        api_endpoint_url=invalid_api_url,
        headers=invalid_headers,
        requests_session=requests_session,
    )
...
def test_get_site_info_valid_api_url_and_invalid_key(
    valid_api_url, invalid_headers, requests_session
):
    # Try to issue a get request using the wrong API endpoint & API Key
    response = get_site_info(
...
def test_get_site_info_invalid_api_url_and_valid_key(
    invalid_api_url, valid_headers, requests_session
):
    # Try to issue a get request using the wrong API endpoint & API Key
    response = get_site_info(
...
def test_get_site_info_valid_api_url_and_valid_key(
    valid_api_url, valid_headers, requests_session
):
    # Try to issue a get request using the wrong API endpoint & API Key
    response = get_site_info(
...
def test_invalid_outages_filter_by_datetime(invalid_outages):
    ...
    assert (
        ...
        == "Cannot perform successful filtering of outages data due to: strptime() argument 1 must be str, not None."
    )
...
def test_valid_outages_filter_with_invalid_site_info(
    valid_filtered_outages_dt, invalid_site_info
):
    ...
    filtered_outages = filter_outages_by_site_info(
        filtered_outages_dt=valid_filtered_outages_dt, site_info_data=invalid_site_info
    )
...
def test_valid_outages_filter_with_valid_site_info(
    valid_filtered_outages_dt, valid_site_info
):
    # Call our function
    filtered_outages = filter_outages_by_site_info(
        filtered_outages_dt=valid_filtered_outages_dt, site_info_data=valid_site_info
...
def test_valid_output_with_valid_filtered_outages(
    valid_final_output, valid_filtered_outages, valid_site_info
):
    # Call our function
    final_output = produce_final_output(
        filtered_outages=valid_filtered_outages, site_info_data=valid_site_info
    )
...
def test_post_invalid_final_output(
    valid_api_url, valid_headers, requests_session, invalid_final_output
):
...
def test_post_valid_final_output(
    valid_api_url, valid_headers, requests_session, valid_final_output
):
    # Try to issue POST request with invalid final output
    response = post_results(
        api_endpoint_url=valid_api_url,
