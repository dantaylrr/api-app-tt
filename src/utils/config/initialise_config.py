...
def _extract_config_values(config_path: str):
    ...
    # AttributeError's are common when parsing dictionaries using .get() - we should aim to catch these
        ...
        logger.error(f"AttributeError occurred when extracting secret name values: {att_ex} - raising RunTime error to prevent further action being taken.")
        raise RuntimeError("There has been an error parsing the configuration file 'config.yaml' - please investigate the logs, refactor & re-try.")
    # We still need to catch any other unexpected errors, e.g. wrong file path provided at entry point
    except Exception as ex:
        logger.error(f"Unexpected error occurred: {ex} - raising RunTime error to prevent further action being taken.")
        raise RuntimeError("There has been an error reading configuration file 'config.yaml' - please investigate the logs, refactor & re-try.")
...
def _extract_ssm_values(ssm_api_url: str, ssm_api_key: str):
    ...
    try:
        ...
        raise RuntimeError(f"Cannot establish boto3 SSM client - attempting to reprocess.")
    ...
    try:
        ...
        logger.error(f"Error exporting SSM parameters to global variables due to: {config_ex}.")
        raise RuntimeError(f"Error exporting SSM parameters to global variables due to: {config_ex}.")
...
def init_config(config_path: str, ssm_flag: bool):
    ...
    if ssm_flag:
        ...
        API_URL, API_KEY = _extract_ssm_values(ssm_api_url = ssm_api_url, ssm_api_key = ssm_api_key)
        ...
        # Catch common os exceptions - env variable doesn't exist
            ...
            logger.error("Environment variables set incorrectly, please reset environment variables before continuing - raising Runtime error to prevent further action.")
            logger.error(f"{env_var_ex}.")
            raise RuntimeError("Environment variables set incorrectly, please reset environment variables before continuing.")